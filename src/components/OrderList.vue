            ...
            <div class="user-data">
                ...
                <div class="my-order-item" v-for="order in order_list">
                    ...
                    <ul class="nav user-data-list" v-for="course in order.course_list">
                        ...
                        <li class="order-status">{{order.order_status}}</li>
                        <li class="order-do">
                            <span class="btn btn2">
                                <router-link :to="'/detail/'+course.id">去学习</router-link>
...
<script>
    ...
    export default {
        ...
        methods: {
           get_order(){
               this.$axios.get(this.$settings.HOST + 'order/order/list/').then(res => {
                   console.log(res);
...
<style scoped>
    .main .banner {
        width: 100%;
        height: 324px;
        background: url(../../static/image/01.png) no-repeat;
